...
const PurchasedCarsPage = () => {
  ...
  useEffect(() => {
    const fetchpurchasedCars = async () => {
      ...
      try {
        if (user) {
          apiBookings = await getBookingbyuser(user?.id);
        }
      } catch (error) {
        console.warn("Failed to fetch API bookings:", error);
      }
      ...
      const userLocalBookings = localBookings.filter((b: any) => b.userId === user?.id);

      // Merge backend and local bookings
      const allBookings = [...(Array.isArray(apiBookings) ? apiBookings : []), ...userLocalBookings];
      
      setpurchasedCars(allBookings);
      setLoading(false);
    };
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 text-black font-sans">
        <div className="max-w-md w-full bg-white rounded-xl border border-gray-200 shadow-sm p-8 text-center space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-xl font-bold text-gray-900">No Bookings Found</h1>
            <p className="text-xs text-gray-500 leading-relaxed">
              You haven't booked any cars yet. Explore our wide range of high-quality certified used cars!
            </p>
          </div>
          <div className="pt-4">
            <Link
              href="/buy-car"
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-4.5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md transition-colors cursor-pointer animate-pulse"
            >
