...
import LocationFilter from "@/components/buy-car/LocationFilter";
import EnhancedSearch from "@/components/buy-car/EnhancedSearch";
import DynamicPriceBadge from "@/components/buy-car/DynamicPriceBadge";
import { parsePrice } from "@/lib/dynamicPricing";
    ...
    const fetchCars = async () => {
      ...
      try {
        ...
        if (car && Array.isArray(car) && car.length > 0) {
          ...
          throw new Error("No cars fetched");
        }
      } catch (error) {
        console.warn("Using mock cars fallback:", error);
        mainCars = MOCK_CARS;
      }

      // Load custom user listed cars from local storage
      const storedCarsRaw = localStorage.getItem("cars24_local_cars");
      const localCars = storedCarsRaw ? JSON.parse(storedCarsRaw) : [];
      
      setCars([...localCars, ...mainCars]);
    };
    fetchCars();
  }, []);

  const { processedCars, isCityFallback } = React.useMemo(() => {
    let result = cars || [];
    let fallback = false;

    if (selectedCity !== "All Cities") {
      const cityLower = selectedCity.toLowerCase();
      const cityMatched = result.filter((car) => {
    ...
    result = result.filter((car) => {
      const priceVal = parsePrice(car.price);
      return priceVal >= priceRange[0] && priceVal <= priceRange[1];
    });

    return { processedCars: result, isCityFallback: fallback };
  }, [cars, selectedCity, selectedBrands, priceRange]);
  return (
          ...
          <div className="md:col-span-3">
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-2xl font-bold font-sans">
                {selectedCity === "All Cities" ? "Used Cars" : `Used Cars in ${selectedCity}`}
              </h1>
            </div>

            <LocationFilter
              onCityChange={setSelectedCity}
              hasCarsMatched={!isCityFallback}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {cars === null
                ...
                : (filteredCars ?? processedCars).map((car) => (
                    <Link
                      key={car.id}
                      href={`/buy-car/${car.id}`}
